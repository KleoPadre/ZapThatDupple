...
import { Download, CheckCircle, HardDrive, Cpu, FileText, Music, WifiOff, RefreshCw } from 'lucide-react'
import { getModels, downloadModel, checkModelUpdates, createWebSocket } from '../utils/api'
...
export function ModelsPage({ t }: { t: any }) {
  ...
  const { settings, setSettings, lang, setModelUpdates } = useStore()
  ...
  const loadModels = async () => {
    try {
      ...
      ;[...data.image, ...data.text, ...data.audio].forEach((m: ModelMeta) => {
        if (m.update_available) updates[m.id] = true
      })
  ...
  useEffect(() => {
    const ws = createWebSocket((msg) => {
      ...
      if (msg.type === 'model_updates_available') {
        loadModels()
    ...
    try {
      ...
      const poll = setInterval(async () => {
        ...
        try {
          ...
          const model = allModels.find((m: ModelMeta) => m.id === modelId)
  ...
  const handleCheckUpdates = async () => {
    ...
    try {
      ...
      setTimeout(() => setCheckingUpdates(false), 10000)
    } catch {
      setCheckingUpdates(false)
    }
  }
  ...
  const ModelCard = ({ model, selected, onSelect, onDownload, isDownloading }: {
    ...
    const sizeStr = sizeMb >= 1000
      ? `${(sizeMb / 1024).toFixed(1)} ${t.models.gb}`
      : sizeMb > 0 ? `${sizeMb} ${t.models.mb}` : '—'

    return (
      <div className={`p-4 rounded-xl border transition-all ${
        selected ? 'border-[#1F555C]/50 bg-[#1F555C]/10' : 'border-[#F2F4F3]/5 bg-[#F2F4F3]/5 hover:border-[#F2F4F3]/10'
      }`}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              {selected && (
                <span className="text-xs text-[#E6E8E6] bg-[#1F555C]/15 px-2 py-0.5 rounded-full">
                  {t.models.selected}
                </span>
              )}
              {model.update_available && (
                <span className="text-xs text-[#E8A838] bg-[#E8A838]/10 px-2 py-0.5 rounded-full">
                  {lang === 'ru' ? 'Есть обновление' : 'Update available'}
                </span>
              )}
            </div>
          ...
          <div className="flex flex-col gap-2 shrink-0">
            {model.hf_id === null ? (
              ...
              <>
                ...
                {model.update_available && (
                  <button onClick={onDownload} disabled={isDownloading}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#E8A838]/10 text-[#E8A838] hover:bg-[#E8A838]/20 transition-all text-xs disabled:opacity-50">
                    {isDownloading ? <span className="animate-spin">↻</span> : <RefreshCw size={12} />}
                    {isDownloading ? t.models.downloading : (lang === 'ru' ? 'Обновить' : 'Update')}
                  </button>
                )}
                {!selected && (
              ...
              <button onClick={onDownload} disabled={isDownloading}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#F2F4F3]/5 text-[#E6E8E6] hover:bg-[#F2F4F3]/10 transition-all text-xs disabled:opacity-50">
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-2xl font-semibold text-[#F2F4F3]">{t.models.title}</h1>
        <button onClick={handleCheckUpdates} disabled={checkingUpdates}
          className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-[#F2F4F3]/5 text-[#F2F4F3]/40 hover:bg-[#F2F4F3]/10 hover:text-[#F2F4F3]/70 transition-all text-xs disabled:opacity-50">
